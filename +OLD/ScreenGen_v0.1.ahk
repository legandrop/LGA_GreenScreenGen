...
GenerateImage:
    ...
    outputDir := scriptDir . "\" . resolution
    ...
    {
        MsgBox, 48, Información, El archivo Verde.jpg ya existe en %resolution%. ¿Deseas sobrescribirlo?
        MsgBox, 36, Confirmación, Por favor, confirma si deseas sobrescribir el archivo existente.
        IfMsgBox, No
            return
    }

    ; Definir ruta para el archivo de error
    errorLog := outputDir . "\error.log"
    outputLog := outputDir . "\output.log"

    ; Construir el comando FFmpeg
    ; Generar una imagen verde usando el filtro color de FFmpeg
    ffmpegCommand := """" . ffmpegPath . """ -f lavfi -i color=c=green:s=" . width . "x" . height . " -frames:v 1 -update 1 """ . outputFile . """ >""" . outputLog . """ 2>""" . errorLog . """"


    ; Opcional: Mostrar el comando para depuración
    ; MsgBox, 64, Depuración, Ejecutando comando:`n%ffmpegCommand%
    ...
    FileAppend, %ffmpegCommand%, %tempBatch%

    ; Ejecutar el archivo de lote de manera oculta
    RunWait, %tempBatch%, , Hide

    ; Verificar si el archivo se creó correctamente
    if FileExist(outputFile)
    {
        MsgBox, 64, Éxito, Verde.jpg ha sido generado en la carpeta %resolution%.
    ...
    ; Opcional: Eliminar los archivos de log después de mostrarlos
    ; FileDelete, %errorLog%
    ; FileDelete, %outputLog%
return
