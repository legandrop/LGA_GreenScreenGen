...
; Función para aplicar tema oscuro a los controles
ApplyDarkTheme(hWnd) {
    DllCall("uxtheme\SetWindowTheme", "ptr", hWnd, "str", "DarkMode_Explorer", "ptr", 0)
}

; Función para aplicar tema oscuro a los menús contextuales
...
; Aplicar tema oscuro a los menús contextuales
contextcolor()
...
Gui, Main:Color, 131313, 060606

Gui, Main:Add, Text, x20 y30 w200 h20 cFFE7C5, Select resolution:
CreateCustomComboBox(20, 50, 210, 21, "Select a resolution", resolutionsList)
...
Gui, Main:Add, Text, x310 y130 w40 h20 vVersionText gShowVersionTooltip cFFE7C5, v1.0

; Aplicar tema oscuro a los botones y al ComboBox
GuiControlGet, hWndCombo, Hwnd, SelectedResolution
ApplyDarkTheme(hWndCombo)
GuiControlGet, hWndButton, Hwnd, Add Resolution
ApplyDarkTheme(hWndButton)
GuiControlGet, hWndButton, Hwnd, Remove Resolution
ApplyDarkTheme(hWndButton)
GuiControlGet, hWndButton, Hwnd, Generate
ApplyDarkTheme(hWndButton)
GuiControlGet, hWndButton, Hwnd, Generate All
ApplyDarkTheme(hWndButton)
...
Gui, Progress:Color, 131313, 060606
Gui, Progress:Font, s10, Verdana
Gui, Progress:Add, Text, vProgressText x20 y20 w300 h20 cFFE7C5, Starting image generation...
Gui, Progress:Add, Progress, vProgressBar x20 y50 w300 h20 Range0-100 cFFE7C5, 0
Gui, Progress:Add, Button, gCancelGeneration x130 y80 w80 h30, Cancel

; Aplicar tema oscuro al botón de cancelar
GuiControlGet, hWndButton, Hwnd, Cancel
ApplyDarkTheme(hWndButton)
...
VarSetCapacity(DWMWA_CAPTION_COLOR, 4, 0)
NumPut(0x131313, DWMWA_CAPTION_COLOR, 0, "UInt")
DllCall("dwmapi\DwmSetWindowAttribute", "Ptr", WinExist(), "UInt", 35, "Ptr", &DWMWA_CAPTION_COLOR, "UInt", 4)
...
GenerateImages:
    Gui, Main:Submit, NoHide
    GuiControlGet, resolutionDisplay,, CustomCombo
    
    if (resolutionDisplay = "Select a resolution")
    {
...
GenerateAll:
    ...
    allResolutions := StrSplit(resolutionsList, "|")

    ; Guardar la selección original
    GuiControlGet, originalSelection,, CustomCombo
    ...
    for index, resolution in allResolutions
    {
        ...
        GuiControl,, CustomCombo, %resolution%
    ...
    GuiControl,, CustomCombo, %originalSelection%
...
; Función para crear un ComboBox personalizado
CreateCustomComboBox(x, y, w, h, options, choiceList) {
    global CustomComboX, CustomComboY, CustomComboWidth, CustomComboHeight
    ...
    ; Crear el control Text para simular el ComboBox sin almacenar en una variable
    Gui, Add, Text, x%x% y%y% w%w% h%h% gShowCustomCombo, %A_Space%%options%
    Gui, Font, s10, Verdana
    Gui, Add, Text, x+5 yp w16 h%h% 0x201 gShowCustomCombo, ▼
    Gui, Font
    CustomComboChoices := choiceList ; Guarda la lista de opciones en una variable global
    return
}
...
ShowCustomCombo:
    if (A_GuiControl = "CustomComboButton")
        ControlClick, CustomCombo
    else
    {
        ControlGet, Vis, Visible,, CustomListBox
        if (!Vis)
        {
            ; Obtener la posición de la ventana principal "GreenScreenGen"
            WinGetPos, WinX, WinY, , , GreenScreenGen

            ; Calcular la posición Y de la lista personalizada
            ListY := WinY + CustomComboY + CustomComboHeight

            ; Agregar mensaje de depuración para verificar las variables
            MsgBox, 64, Depuración, WinX: %WinX%`nWinY: %WinY%`nCustomComboX: %CustomComboX%`nCustomComboY: %CustomComboY%`nCustomComboWidth: %CustomComboWidth%`nListY: %ListY%

            ; Crear y mostrar la GUI personalizada
            Gui, CustomList:New, +ToolWindow -Caption +AlwaysOnTop
            Gui, CustomList:Color, 1F1F1F, 2D2D2D
            Gui, CustomList:Font, s10 cFFFFFF, Verdana
            Gui, CustomList:Add, ListBox, x0 y0 w%CustomComboWidth% r10 vCustomListBox gCustomListBoxHandler AltSubmit, %CustomComboChoices%
            
            ; Mostrar la GUI personalizada con las opciones correctas
            Gui, CustomList:Show, x%WinX% y%ListY% w%CustomComboWidth% NoActivate, CustomListBox
        }
        else
            Gui, CustomList:Hide
    }
...
CustomListBoxHandler:
    if (A_GuiEvent = "DoubleClick")
    {
        GuiControlGet, Selected,, CustomListBox
        GuiControl,, CustomCombo, % StrSplit(CustomComboChoices, "|")[Selected]
        Gui, CustomList:Hide
    }
return